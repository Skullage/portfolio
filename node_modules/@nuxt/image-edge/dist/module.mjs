...
import { useNuxt, createResolver, resolvePath, defineNuxtModule, addComponent, addTemplate, addPlugin } from '@nuxt/kit';
...
async function resolveProvider(_nuxt, key, input) {
  ...
  input.provider = BuiltInProviders.includes(input.provider) ? require.resolve("./runtime/providers/" + input.provider) : await resolvePath(input.provider);
